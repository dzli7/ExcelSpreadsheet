...
class MoveCellStressTest(unittest.TestCase):
    def test_move_cell_row_20(self):
        wb = Workbook()
        p = Profiler()
        ...
        p.start()
        wb.move_cells(name, 'A1', f'A{N}', 'B1')
        p.stop()

        p.print(show_all=True)

    def test_move_cell_row_100(self):
        wb = Workbook()
        p = Profiler()
    ...
    def test_move_cell_2000(self):
        ...
        p.start()
        wb.move_cells(name, 'A1', f'{cmax}{M}', f'{cmax1}1')
        p.stop()

        with open('logs/project5-perf-log-movecell.txt', 'w') as f:
